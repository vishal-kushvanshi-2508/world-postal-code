...
def create_table_country_url():
    ...
    # delete_query = f"DROP TABLE IF EXISTS {country_url_table_name}"
...
def create_table_region():
    ...
    ## delete table if exist ....
    # delete_query = f"DROP TABLE IF EXISTS {region_table_name}"
    # cursor.execute(delete_query)
...
def fetch_area_table_data():
    connection = get_connection()
    cursor = connection.cursor()
    query = f"SELECT * FROM {area_table_name} WHERE status = 'pending';"
    cursor.execute(query)
...
def postal_data_insert(list_data : list):
    ...
    try:
        # cursor.execute(f"SELECT COUNT(*) FROM {postal_table_name}")
        # total_contry_rows = cursor.fetchone()[0]
        # print("total table recodes : ", total_contry_rows)
        # if total_contry_rows == 0:
            # if table is empty then inset data in table.
